...
const DASHBOARD_DATA = {
  ...
  telemetry: {
    temperature: { label: "Temperature", sp: 910, unit: "°C", threshold: 0.1 },
    cp: { label: "Carbon Potential", sp: 0.75, unit: "", threshold: 0.05 },
    oil: { label: "Quench Oil Temperature", sp: 100, unit: "°C", threshold: 2 },
  },
  quenchStatus: [
    {
      id: "oil-temp",
...
const generateNewPoint = (index) => {
  ...
  const oilPV = parseFloat((95 + Math.random() * 7).toFixed(1));
              ...
              >
                ...
                >
                  ...
                  <span className="text-[9px] opacity-60">
                    / {DASHBOARD_DATA.system.totalTime}
                  </span>
                </span>
              </div>
              <div
            ...
            <div className="col-span-12 lg:col-span-4 flex flex-col gap-10 h-full">
              ...
              <div className="grid grid-cols-2 gap-gutter">
                {DASHBOARD_DATA.flowPanels.map((panel, idx) => (
                  <div
                    key={idx}
                    className="bg-surface-container rounded-xl p-card-padding border border-surface-variant"
                  >
                    ...
                    {panel.sp ? (
                      <div className="flex flex-col gap-1 mt-2">
                        ...
                        <div className="flex justify-between text-[12px]">
                          <span className="text-slate-500 font-bold">SP</span>
                          <span className="font-bold">{panel.sp}</span>
                        </div>

                        {/* PV */}
                        <div className="flex justify-between text-[12px]">
                          <span className="text-slate-500 font-bold">PV</span>
                          <span className="font-bold">{panel.pv}</span>
                        </div>

                        {/* DEV */}
                        <div className="flex justify-between text-[12px]">
                          <span className="text-slate-500 font-bold">DEV</span>
                          <span
                            className={`font-bold ${
                      ...
                      <div className="flex items-baseline gap-2">
                        <span
                          className={`font-data-display text-data-display text-${panel.color}`}
                        >
                          {panel.value}
                          <span className="text-lg">{panel.unit}</span>
                        </span>
                      </div>
                    )}
                ...
                >
                  {DASHBOARD_DATA.deviations.active.map((dev, idx) => (
                    ...
                    >
                      {/* Left Side */}
                      <div
                        className={`w-1 self-stretch bg-${dev.color}-500 rounded-full`}
                      ></div>
                  ...
                  <div className="p-3 flex gap-2 items-center">
                    ...
                    <div className="flex-1 flex flex-col gap-2">
                      <p className="text-[15px] font-bold text-on-surface leading-tight">
                        ...
                        <span
                          className={`text-${DASHBOARD_DATA.prediction.status.toLowerCase()}-500 drop-shadow-[0_0_5px_rgba(239,68,68,0.8)]`}
                        >
                      ...
                      <div className="w-12 h-12 relative flex items-center justify-center mb-1">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <span className="text-[12px] font-bold text-on-surface leading-none">
                            {DASHBOARD_DATA.prediction.confidence}
            ...
            >
              ...
              <div className="flex justify-between items-start">
                  ...
                  {DASHBOARD_DATA.timeline.stages.map((stage, idx) => (
                    <React.Fragment key={stage.id}>
                      ...
                      >
                        ...
                        <span
                          className={`text-[9px] ${stage.status === "active" ? "text-primary" : "text-outline"} font-mono`}
                        >
                          {stage.progress}
                        </span>
                      </div>
                      {idx < DASHBOARD_DATA.timeline.stages.length - 1 && (
                        <div
                          className={`flex-1 mx-1.5 border-t ${stage.status === "completed" ? "border-outline-variant/30" : "border-outline-variant/20"} relative min-w-[15px]`}
                        ></div>
                      )}
                    </React.Fragment>
                  ))}
                ...
                >
                  ...
                  >
                    {DASHBOARD_DATA.timeline.cycleTime}{" "}
                    <span className="text-outline text-xs">
                      / {DASHBOARD_DATA.timeline.targetCycleTime}
                    </span>
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col mt-3 relative">
                ...
                <div className="relative w-full h-10 flex items-center px-5">
                  ...
                  {/* Current Head */}
                  <div
                    className={`absolute left-[${DASHBOARD_DATA.timeline.headPos}] w-[3px] h-6 bg-primary shadow-[0_0_10px_rgba(173,198,255,1)] -translate-x-1/2 -translate-y-1/2 top-1/2 z-20 rounded-full`}
                  >
                ...
                <div className="flex justify-between w-full mt-4 px-5 text-[11px] text-on-surface-variant font-mono font-bold">
                  {DASHBOARD_DATA.timeline.scale.map((time, idx) => (
                    <span
                      key={idx}
                      className={`w-10 text-center ${idx === 0 ? "-ml-5" : idx === DASHBOARD_DATA.timeline.scale.length - 1 ? "-mr-5" : ""}`}
                    >
                      {time}
                    </span>
                  ))}
              ...
              >
                {DASHBOARD_DATA.timeline.legend.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <div
                      className={`w-3 h-3 rounded-full bg-${item.color}-500 shadow-[0_0_8px_rgba(var(--${item.color}-glow),0.6)] ${item.pulse ? "animate-pulse" : ""}`}
                    ></div>
                    <span
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-gutter mt-10">
              {DASHBOARD_DATA.kpis.map((kpi) => (
                ...
                >
                  ...
                  >
                    <span
                      className={`material-symbols-outlined text-${kpi.color} text-xl`}
                    >
                      {kpi.icon}
                    </span>
                  </div>
                    ...
                    >
                      {kpi.id === "util"
                        ...
                        : kpi.id === "energy"
                          ? (
                              452 +
                              Math.cos(currentTime.getSeconds() / 3) * 5
                            ).toFixed(0)
                          : kpi.value}
